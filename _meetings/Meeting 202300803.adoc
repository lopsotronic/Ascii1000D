...
digraph g {
  ...
  rankdir="TD";
    "Is it DITA" -> "Is it same DITA version" -> "Convert topic type?" -> "Old DITAMAPs work" -> "Styler works with old .style files?" -> "DITA-OT for SCORM" -> "Process graphics library" -> "Testing"
